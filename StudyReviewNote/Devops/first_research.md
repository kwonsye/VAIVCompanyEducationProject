...
- 처음에는 기존의 목표를 달성하기 위한 방향을 위한 많은 툴들을 찾아보는 것으로 시작했는데, 공부하다보니 너무 빌드,테스트, 운영 자동화, 컨테이너 가상화로 빠진 것 같긴 했다..

- 일단 1차는 이렇게 했지만 처음부터 너무 큰그림으로 완벽하게 구축하려 하지 말고, 일차적으로 목표를 이루는 방향으로 문제를 작게 쪼개서 생각해봐야할 수도 있을 것 같다..다른 큰 회사들도 데브옵스를 구축하는데 몇 개월씩 걸렸다고 하니까 큰 그림을 한번에 적용시키기는 무리일 수 도 있을 거라는 생각이 들었다.
