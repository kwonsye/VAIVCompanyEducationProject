...
public class WordCount {
    ...
    private static final String INPUT_DATA_DIRECTORY_PATH = "input";
    private static final String OUTPUT_DATA_DIRECTORY_PATH = "output";
    ...
    public static class TokenizerMapper
        ...
        Writable : DataInput과 DataOutput을 기반으로한 시리얼라이즈 오브젝트
    ...
    private static Job setJobsConfigurations() throws IOException {
        ...
        * 각각의 node에서 Mapper instance가 산출한 데이터를 입력받고 Combiner 가 지정된 작업을 하면 그 결과물을 Reducer 에게 보냄
        * Combiner 는 일종의 “mini-reduce” 프로세스로서 하나의 단위 컴퓨터에서 생성된 데이터만을 대상으로 함
        * node 별로 중간합산 하여 각 단어 당 한번씩만 Reducer에 전달되므로 shuffle 프로세스에서 요구되는 대역폭을 획기적으로 줄일 수 있게 되고 결과적으로 job 처리속도가 개선됨
        */
        job.setCombinerClass(IntSumReducer.class); //setCombinerClass(combiner : Class<? extends Reducer>) Combiner는 Reducer interface를 확장해서 만들 수 있음
        job.setReducerClass(IntSumReducer.class);
        job.setOutputKeyClass(Text.class); // 최종 OUTPUT_KEY_TYPE == reducer 의 OUTPUT_KEY_TYPE
        job.setOutputValueClass(IntWritable.class); // 최종 OUTPUT_VALUE_TYPE == reducer 의 OUTPUT_VALUE_TYPE
